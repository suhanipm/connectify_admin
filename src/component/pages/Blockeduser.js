...
function Blockeduser(){
    ...
    useEffect(() => {
      const fetchUsers = async () => {
        try {
          const response = await fetch('http://localhost:5038/api/social_media/admin/blocked-users');
        ...
        <tbody>
          {users.map((user, index) => (
            <tr key={index}>
              ...
              <td>
                <button onClick={() => handleBlock(user._id)} className="delete-button">Unblock</button>
              </td>
            </tr>
          ))}
        </tbody>
